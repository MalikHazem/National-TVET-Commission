        ...
        <div class="col-lg-12 col-md-10">
            ...
            <table id="myTable" dir="rtl" data-aos="fade-up">
                ...
                @foreach ($schools as $school)
                        <tr class="tr-td" onclick="window.location='{{url('/الهيئة/School')}}';">
                            <td>{{$school->id}}</td>
                            <td>{{$school->name}}</td>
                            <td>{{$school->category}}</td>
                            <td>{{$school->date_create}}</td>
                            <td>{{$school->phone}}</td>
                            <td>{{$school->website}}</td>
                            <td colspan="2" style="text-align: center"><a href="{{url('/Education/School/edit',$school->id)}}" class="w3-padding w3-xlarge w3-text-cyan"><i class="material-icons">edit</i></a>
                            <a href="{{url('/Education/School/destroy',$school->id)}}" class="w3-padding w3-xlarge w3-text-red"><i class="material-icons">delete</i></a></td>
                        </tr>
                @endforeach
              </table>
