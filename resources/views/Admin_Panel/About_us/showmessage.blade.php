            ...
            <table id="myTable" dir="rtl" data-aos="fade-up">
                ...
                @foreach ($Mess as $Mes)
                        <tr class="tr-td" onclick="window.location='{{url('/الهيئة/About_us/VisionandMessage')}}';">
                            <td>{{$Mes->subject}}</td>
                            <td colspan="2" style="text-align: center"><a href="{{url('/About_us/Message/edit',$Mes->id)}}" class="w3-padding w3-xlarge w3-text-cyan"><i class="material-icons">edit</i></a></td>
                        </tr>
                @endforeach
              </table>
